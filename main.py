...
hourly_diff = temp_df.groupby("시")["기온차"].mean().reset_index()
hourly_diff = hourly_diff.set_index("시")
...
monthly_diff = temp_df.groupby("월")["기온차"].mean().reset_index()
monthly_diff = monthly_diff.set_index("월")
...
```
scatter_df = power_df[["서울", "전력수요"]].dropna()
...
temp_power = power_df.groupby("기온구간")["전력수요"].mean().reset_index()

# 문자열로 변환 (이거 중요)
temp_power["기온구간"] = temp_power["기온구간"].astype(str)
temp_power = temp_power.set_index("기온구간")
...
monthly_power = power_df.groupby("월")["전력수요"].mean().reset_index()
